...
#else
    feedrate = homing_feedrate[axis]/15 ;
#endif
    plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], feedrate/60, active_extruder);
    st_synchronize();
#ifdef DELTA
...
case 404:  //M404 Enter the nominal filament width (3mm, 1.75mm ) N<3.0> or display nominal filament width 
	...
	skip_pause:
      ...
      cancel_heatup = false;
		...
		if (active_extruder > 0)
		{
		current_position[X_AXIS]+=extruder_offset[0] [1];
		current_position[Y_AXIS]+=extruder_offset[1] [1];
		}
		...
		plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], lastpos[E_AXIS], 10, active_extruder); //unretract
		...
		plan_buffer_line(lastpos[X_AXIS], lastpos[Y_AXIS], lastpos[Z_AXIS], lastpos[E_AXIS], 100, active_extruder); //final unretract
